...
private fun DiagnosticsPanel(state: MainUiState, onRetryLightSensor: () -> Unit) {
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            MetricRow("decisionCurrentPercent", state.runtime.lastDecisionCurrentPercent?.let { "%.1f%%".format(it) } ?: "-")
            MetricRow("decisionDeltaPercent", state.runtime.lastDecisionDeltaPercent?.let { "%.1f%%".format(it) } ?: "-")
            MetricRow("decisionSystemDelta", state.runtime.lastDecisionSystemDelta?.toString() ?: "-")
            MetricRow("deadbandPercent", state.runtime.lastDecisionDeadbandPercent?.let { "%.1f%%".format(it) } ?: "-")
            MetricRow("minSystemDelta", state.runtime.lastDecisionMinSystemDelta?.toString() ?: "-")
            MetricRow("throttleMs", state.runtime.lastDecisionThrottleMillis?.toString() ?: "-")
            MetricRow("elapsedSinceLastWriteMs", state.runtime.lastDecisionElapsedMillis?.toString() ?: "-")
